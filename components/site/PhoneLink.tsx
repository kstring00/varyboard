...
export function PhoneLink({ className = "", compact = false }: { className?: string; compact?: boolean }) {
  return (
    <a href={brand.phoneHref} aria-label={`Call ${brand.phone}`} className={`inline-flex min-h-12 min-w-12 items-center justify-center gap-2 whitespace-nowrap font-semibold ${className}`}>
